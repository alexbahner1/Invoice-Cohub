class PdfMaker
  def self.make(data:, path:)
    ...
    Prawn::Document.generate(path, page_size: 'LETTER', page_layout: :portrait) do
      ...
      create_stamp 'totals' do
        ...
        grid([34, 2], [39, 2]).bounding_box do
          ...
          grid(0, 0).bounding_box do
            # transparent(0.05) { stroke_bounds }
            bounding_box([0, cursor], :width => 85, :height => 11) do
              self.line_width = 1
              stroke_color 100, 100, 100, 1
              move_down 1
              indent(3) do
          ...
          grid(0, 1).bounding_box do
            bounding_box([0, cursor], :width => 80, :height => 11) do
      ...
      if invoice.items.count < 13
        ...
        # Aubsequent Pages of items
        # invoice.items.where.not(id: first_page_items.pluck(:id)).order(created_at: :asc).find_in_batches(batch_size: 12) do |batch|
        invoice.items.select { |item| !first_page_items.map(&:id).include? item.id }.each_slice(12) do |batch|
          start_new_page
          define_grid(columns: 1, rows: 8, gutter: 10)
